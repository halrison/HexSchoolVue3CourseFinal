<template>
        ...
        <table class="table table-striped">
            <thead>
                <tr class="row mx-0">
                    ...
                    <th class="col-lg-2 text-center">動作</th>
                </tr>
            </thead>
            <tbody>
    ...
    <Pagination :pagination="pagination" v-show="pagination.total_pages > 1"
                @paginate=" getCoupons" />
    <Modal ref="couponModal" :title="`${ coupon.id ? '編輯' : '新增' }優惠券`">
        <FormC v-slot="{ errors }" @submit="addEditCoupon" :initial-values="coupon" :initial-errors="{}">
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="col-3 mb-2">
                        ...
                        <FieldC name="enabled" class="form-check-input" type="checkbox"
                               :checked="coupon.is_enabled" :true-value="1" :false-value="0" />
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <input type="submit" class="btn btn-primary" value="確定" />
                <input type="reset" class="btn btn-secondary" @click="$refs.couponModal.hideModal()" value="取消" />
            </div>
        </FormC>
    </Modal>
    <Modal ref="removeModal" title="移除優惠券">
        <div class="modal-body">
            <p>您要移除{{ coupon?.title }}嗎？</p>
        </div>
        <div class="modal-footer">
    ...
    export default {
        ...
        methods: {
            ...
            openModal (action, item) {
                if (action === 'add') {
                    this.coupon.due_date = this.$filters.transDate(new Date())
                    this.$refs.couponModal.showModal()
                } else if (action === 'modify') {
                    this.coupon = {
                        ...item,
                        due_date:this.$filters.transDate(item.due_date)
                    }
                    this.$refs.couponModal.showModal()
            ...
            removeCoupon () {
                this.$http.delete(`${process.env.VUE_APP_API}/api/${process.env.VUE_APP_PATH}/admin/coupon/${this.coupon.id}`)
                    .then(({data}) => {
                        if (data.success) {
                            this.emitter?.emit('message', {title: '移除優惠券成功', message: data.message, type: 'success'})
                            this.hideModal()
                        } else {
                            this.emitter?.emit('message', {title: '移除優惠券失敗', message: data.message, type: 'warning'})
                        }
                    }).catch(err => {
                        this.emitter?.emit('message', {title: '移除優惠券發生錯誤', message: err, type: 'danger'})
                    })
            }
        },
        data () {
